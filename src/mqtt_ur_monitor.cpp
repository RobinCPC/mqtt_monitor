#include <mqtt_monitor/mqtt_ur_monitor.h>
#include "../include/mqtt_monitor/mqtt_ur_monitor.h"

MqttUrMonitor::MqttUrMonitor(ros::NodeHandle nh)
  : nh_(nh)
{
  ...
  //ur_mod_sub = nh_.subscribe("/pub_I0", 1, &MqttUrMonitor::ur_msg_callback, this);
  ur_jnt_pub = nh_.advertise<sensor_msgs::JointState>("/ur/joint_states", 1);
  //ur_mqtt_pub = nh_.advertise<std_msgs::String>("/mqtt_ur_data", 1);
  ur_mqin_sub = nh_.subscribe("/mqin_ur_data", 1, &MqttUrMonitor::ur_mqin_callback, this);

  //// Publish total connecting time
  //time_pub = nh_.advertise<std_msgs::String>("/mqtt_time", 1);
}

MqttUrMonitor::~MqttUrMonitor(){}

//void MqttUrMonitor::ur_msg_callback(const robot_gateway::ur_msg& ur_mod_msg)
//{
//  //publish UR joint state for local RViz
...
//    ur_mqtt_pub.publish(ur_data_msg);
//
//  }
//
//  return;
...
void MqttUrMonitor::ur_mqin_callback(const std_msgs::String& ur_mqin_msg)
{
...
void MqttUrMonitor::pub_joint_state(const std::vector<double>& jntVals)
{
  ...
  ur_jnt_pub.publish(jointStateMsg);

  return;
}

std::vector<std::string> MqttUrMonitor::split(std::string cmd, std::string delimiter)
{
